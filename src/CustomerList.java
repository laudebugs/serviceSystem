...
public class CustomerList {
	...
	public static void setPrevioushour(int hour) {
		CustomerList.previoushour = hour;
	}
	...
	public class Customer {
		...
		public Customer(String info) {
			...
			int extra_time = 0;
			if (Integer.parseInt(times[1].trim())>previoushour && !CustomerList.pm) {
				CustomerList.setPrevioushour(Integer.parseInt(times[1].trim()));
			}
			else if (Integer.parseInt(times[1].trim())<previoushour && !CustomerList.pm){
				CustomerList.setPm(true);
			}
			int time = Integer.parseInt(times[1].trim())*3600+Integer.parseInt(times[2].trim())*60+Integer.parseInt(times[2].trim());

			if (CustomerList.isPm()) {
				time+= 12*3600;
			}
	...
	public void addLast(Customer c) {
		...
		if (newNode.data.getEntry_time()-temp.position.data.getEntry_time()<CustomerList.serveTime//If time difference is less than service time
				) {
			newNode.data.wait_time=temp.position.data.wait_time+(CustomerList.serveTime-(newNode.data.getEntry_time()-temp.position.data.getEntry_time()));
			CustomerList.currentQueue++;
		}
		else {
			/*
			 * Look through this
			...
			CustomerList.idletime+=time;
			if (time>CustomerList.longestIdleTime) {
				CustomerList.longestIdleTime=time;
			}
			newNode.data.wait_time=0;
			if(CustomerList.currentQueue>CustomerList.longestQueue) {
				CustomerList.longestQueue = CustomerList.currentQueue;
				CustomerList.currentQueue = 0;
			}
		}
	...
	public static void main(String[]args) throws FileNotFoundException{
		 ...
		 */
		try {
			CustomerList.serveTime = Integer.parseInt(readCustomers.readLine());
		...
		try {
			String thisLine = "";
			result += readQueries.readLine()+ ": "+CustomerList.totalCustomers +"\n";
			result += readQueries.readLine()+ ": "+CustomerList.longestIdleTime+"\n";
			result += readQueries.readLine()+ ": "+CustomerList.idletime+"\n";
			result += readQueries.readLine()+ ": "+CustomerList.longestQueue+"\n";
